...
checkForcedMoves() {
    ...
    while (counter < maxIterations) {
        ...
        let emptySpaces = this.findEmptyAdjacentSpaces(); // Find empty spaces *inside* the loop
        ...
        for (let { x, y } of emptySpaces) {
            ...
            if (forcedTile) {
                ...
                console.log("Forced move(s) made (Iteration", counter, "):", forcedMovesThisIteration);
                console.log("Board state:", JSON.stringify(this.board.tiles));
                emptySpaces = this.findEmptyAdjacentSpaces();//Check for additional forced moves in this iteration
                break; // Exit inner loop immediately
            }
        }

        if(forcedMovesThisIteration.length === 0){
            break;//If no forced moves are found, exit the while loop
        }

        if (counter === maxIterations) {
            console.error("Potential infinite loop in checkForcedMoves! Reached max iterations.");
            break; // Exit the loop to prevent a freeze
        }
    }
}
    ...
    for (let { x, y } of emptySpaces) {
        for (let tileType of ['curve', 'cross']) {
            ...
            for (let rotation of rotations) {
                ...
                let originalBoard = this.board.cloneBoard();
                this.board.placeTile(x, y, tile);
                this.game.checkForcedMoves(); // Handle forced moves AFTER placing the opponent's tile

                //Now check the forced moves after the opponent makes the move.
                let forcedMoves = this.game.findEmptyAdjacentSpaces().filter(({x,y}) => this.game.findForcedTile(x,y) !== null);

                //Simulate the forced moves for the AI player.
                let originalBoardAfterForced = this.board.cloneBoard();
                for(let {x,y} of forcedMoves){
                    let forcedTile = this.game.findForcedTile(x,y);
                    this.board.placeTile(x,y,forcedTile);
                }
                ...
                this.board.tiles = originalBoard.tiles; // Restore the board to BEFORE opponent's move
                this.game.gameState = "game";
                this.game.winningPlayer = null;
                ...
                if (canWin) {
                    ...
                    console.log("canOpponentWinNextMove: Opponent CANNOT win with this move.");
                }
            }
        }
    }
